...
/// Service that manages the synchronization of health measurements 
...
class SyncService {
  ...
  // Endpoints
  static const String _cloudFunctionUrl = 'https://us-central1-your-project-id.cloudfunctions.net/receiveHealthData';
  ...
  /// Last time successful sync occurred
  ...
  Future<void> initialize() async {
    ...
    try {
      ...
      // Check for pending measurements to sync
      final pendingCount = await _getPendingMeasurementCount();
      if (pendingCount > 0) {
        debugPrint('Found $pendingCount pending measurements to sync');
        // Start a background sync after initialization
        _triggerSyncAfterDelay();
      }
    } catch (e) {
  ...
  /// Store a health measurement for syncing
  Future<void> storeMeasurement(HealthMeasurement measurement) async {
    await _ensureInitialized();
    
    try {
      // Generate a unique ID if not provided
      final measurementId = measurement.id ?? const Uuid().v4();
      ...
      debugPrint('Stored health measurement for future sync: ${measurement.type}');
      
      // Notify listeners
      _syncStatusController.add(SyncStatus(
        pendingCount: await _getPendingMeasurementCount(),
      ...
      debugPrint('Error storing measurement: $e');
    }
  }
  
  /// Schedule a sync after a short delay
  /// This helps batch multiple measurements together
  void _triggerSyncAfterDelay() {
    if (_isSyncing) return;
    
    // Wait 5 seconds to batch measurements together
  ...
  /// Manually trigger synchronization of pending measurements
  Future<bool> syncMeasurements() async {
    await _ensureInitialized();
    ...
    try {
      ...
      final List<Map<String, dynamic>> pendingData = await _database!.query(
        'measurements',
        where: 'sync_status = ?',
        whereArgs: ['pending'],
      ...
      debugPrint('Attempting to sync ${pendingData.length} measurements');
      
      // Convert to HealthMeasurement objects
      final List<HealthMeasurement> measurements = pendingData.map((data) {
        return HealthMeasurement(
          ...
          metadata: data['metadata'] != null ? 
            Map<String, dynamic>.from(jsonDecode(data['metadata'] as String)) : null,
        );
      }).toList();
      ...
      if (response.statusCode == 200) {
        // Parse response to handle individual successes/failures
        ...
        if (responseData['success'] == true && responseData['results'] != null) {
              ...
              await _database!.update(
                'measurements',
                {
                  ...
                  'retry_count': Sqflite.firstIntValue(await _database!.rawQuery(
                    'SELECT retry_count FROM measurements WHERE id = ?', [id])) ?? 0 + 1,
                },
                where: 'id = ?',
                whereArgs: [id],
              );
          ...
          debugPrint('Successfully synced ${results.length} measurements');
          success = true;
        } else {
          // Handle batch failure
          debugPrint('Sync failed: ${response.body}');
          await _markBatchAsFailed(pendingData, 'Batch sync failed: ${responseData['error'] ?? 'Unknown error'}');
        }
      } else {
        // Handle server error
        ...
        try {
          ...
          errorMessage = errorData['error'] ?? 'Server returned ${response.statusCode}';
        } catch (e) {
          errorMessage = 'Server returned ${response.statusCode}: ${response.body}';
        }
        
        await _markBatchAsFailed(pendingData, errorMessage);
      }
    } catch (e) {
      // Handle exceptions
      if (e is SocketException) {
        debugPrint('Network error during sync: $e');
      } else if (e is TimeoutException) {
        debugPrint('Sync timed out: $e');
      } else {
  ...
  /// Mark a batch of measurements as failed
  Future<void> _markBatchAsFailed(List<Map<String, dynamic>> measurements, String error) async {
    // Update all measurements in the batch
    for (var measurement in measurements) {
      final String id = measurement['id'] as String;
      final int retryCount = (measurement['retry_count'] as int?) ?? 0;
  ...
  Future<void> _initDatabase() async {
    ...
    final path = join(databasesPath, 'health_measurements.db');
    ...
    _database = await openDatabase(
      ...
      onCreate: (db, version) async {
        ...
        await db.execute(
          'CREATE INDEX sync_status_idx ON measurements (sync_status)');
      },
    );
  ...
  /// Get the count of pending measurements
  Future<int> _getPendingMeasurementCount() async {
    if (_database == null) return 0;
    
    final result = await _database!.rawQuery(
      'SELECT COUNT(*) FROM measurements WHERE sync_status = ?', 
      ['pending']
    );
  ...
  Future<void> retryFailedMeasurements() async {
    await _ensureInitialized();
    
    try {
      // Update failed measurements to pending
      await _database!.update(
        'measurements',
        {'sync_status': 'pending'},
        where: 'sync_status = ?',
      ...
      debugPrint('Marked failed measurements for retry');
      
      // Start sync
      _triggerSyncAfterDelay();
    } catch (e) {
      debugPrint('Error retrying failed measurements: $e');
    }
  }
